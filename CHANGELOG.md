...
- **SKILL.md 编排层其余修正**：① 环境探测补"项目级优先、先命中先用"的重复安装裁决（此前五个候选目录只列顺序无优先级声明）；② 缺 skill 处置从"缺一全停"改为按任务链懒判定（纯营销项目缺 interface-design 可继续，路由到才必须装）；③ 豁免章节矛盾修复——前文"design-system-* 没有豁免"与后文把 design-system-* 放进 ignore-value 示例并存，现统一为"四条 design-system 规则一律不豁免"；④ Taste 约束指令补官方设计系统例外（Material/Fluent/Carbon 走官方包，不拿 DESIGN.md token 强刷官方组件，例外本身记录进 DESIGN.md）与字体加载条款（生产环境 next/font 或 self-host，`css_import` 仅开发期便利）；⑤ interface-design 约束第 1 条补明 system.md 保存环节的落点重定向到 `## Components`；⑥ "10 色角色"表述统一修正为"UUPM 出 10 色 + 0.3 补 cta = 11 色"；⑦ 删除/弱化全部无支撑的绝对化措辞（"100% stable"、"机械保证"等），README 双语同步。理由：编排层文档的每一处失真都是使用方 agent 的错误指令。
