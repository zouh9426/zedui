...
4. **`python3 <zedui>/scripts/doctor.py --skill-home zedui=<zedui> --skill-home ui-ux-pro-max=<ui-ux-pro-max> --skill-home design-taste-frontend=<design-taste-frontend> --skill-home interface-design=<interface-design> --skill-home impeccable=<impeccable>`** — full-chain installation health check: the five skill resolutions, impeccable version and public detector entry, a real UUPM `--design-system --json` contract probe, and zedui script compilation. You already resolved the five real paths in Steps 2/3 — pass them explicitly so doctor checks the exact copies your host actually loads (an invalid explicit path fails closed; there is no silent fallback to auto-discovery). If the authoritative paths genuinely can't be determined, doctor still runs with no arguments and auto-discovers, but results marked `[auto-discovered]` are a generic fallback — with duplicate installs they are not guaranteed to match the host's actual selection. **The installation counts as complete only when all critical checks pass.** Report ⚠️ warnings to me as-is (e.g. impeccable version differing from the tested baseline, duplicate-install notices), but a ✗ critical failure must be fixed and re-run — never treat it as a successful install.
