...
4. **`python3 <zedui>/scripts/doctor.py --skill-home zedui=<zedui> --skill-home ui-ux-pro-max=<ui-ux-pro-max> --skill-home design-taste-frontend=<design-taste-frontend> --skill-home interface-design=<interface-design> --skill-home impeccable=<impeccable>`** —— 全链路安装体检：五个 skill 解析、impeccable 版本与公开检测入口、UUPM `--design-system --json` 真实契约探测、zedui 脚本可编译。你在第 2/3 步已经解析出五个真实路径，显式传入让 doctor 体检的就是宿主实际加载的那五份（显式路径无效会 fail-closed 报错，不会退回自动发现）。如果确实拿不到确定路径，doctor 也可无参运行自动发现，但输出里 `[auto-discovered]` 标注的结果只是通用 fallback——多副本时不保证等于宿主实际选择。**critical checks 全过才算安装完成**；⚠️ warning（如 impeccable 版本与实测基线不同、重复安装提示）如实向我汇报即可，但 ✗ critical failure 必须修复后重跑，不能当安装成功。
