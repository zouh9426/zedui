...
3. **五确认项照旧**：0.3 的次级文字色 / 中性墨色 / 暗色 token / CJK 字体栈 / 等宽字体栈逐项检查，缺则补——旧规范最常缺的正是这几项。**补建议 token 前先对照页面实现值**：detector 的 `design-system-color` 有颜色通道容差（±6）——建议值若与页面既有实现色接近，会把真实的未登记色"无意洗白"判为色板内、不再报漂移。拿不准的页面色保持漂移状态更安全。
